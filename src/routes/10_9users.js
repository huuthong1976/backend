...
router.get('/', protect, authorize(['Admin']), async (req, res) => { 
  try {
    const [users] = await db.query('SELECT id, full_name, email, role, employee_code, is_active FROM employees');
    res.json(users);
  } catch (err) {
...
router.post('/', protect, authorize(['Admin']), async (req, res) => { 
  const { full_name, email, role, employee_code, password_hash } = req.body;
  try {
    // Kiểm tra xem username hoặc email đã tồn tại chưa
    const [existingUser] = await db.query('SELECT id FROM employees WHERE full_name = ? OR email = ?', [full_name, email]);
    if (existingUser.length > 0) {
        return res.status(400).json({ msg: 'Username hoặc Email đã tồn tại.' });
    }

    const hashed = await bcrypt.hash(password_hash || 'admin@123', 10);
    await db.query(
      'INSERT INTO employees (full_name, email, role, employee_code, password_hash, is_active) VALUES (?, ?, ?, ?, ?, 1)',
      [full_name, email, role, employee_code, password_hash]
    );
...
router.put('/:id', protect, authorize(['Admin']), async (req, res) => { 
  const { full_name, email, role, employee_code, is_active } = req.body;
  try {
    // Optional: Kiểm tra username/email mới có bị trùng với người khác không (trừ chính user đang update)
    await db.query(
      'UPDATE employees SET full_name = ?, email = ?, role = ?, employee_code = ?, is_active = ? WHERE id = ?',
      [full_name, email, role, employee_code, is_active, req.params.id]
    );
...
router.put('/:id/reset-password', protect, authorize(['Admin']), async (req, res) => { 
  try {
    ...
    await db.query('UPDATE employees SET password_hash = ? WHERE id = ?', [hashed, req.params.id]);
